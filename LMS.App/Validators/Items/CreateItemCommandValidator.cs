...
{
    public CreateItemCommandValidator(IStringLocalizer<ErrorMessages> localizer)
    {
        ...
        RuleForEach(x => x.Dto.Values).ChildRules(value =>
        {
            ...
            value.RuleFor(v => v)
                .Must(v => !string.IsNullOrEmpty(v.ValueText) || !string.IsNullOrEmpty(v.ValueUri) || v.ValueResourceId.HasValue)
                .WithMessage(localizer["ValueMustHaveData"]);
        });
    }
}